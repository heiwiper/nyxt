...
(define-parenscript set-hintable-attribute (selector)
  (ps:flet ((get-all-elements (root &optional elements)
              (ps:when (ps:equal (ps:@ elements) ps:nil)
                (ps:setf (ps:@ elements) (ps:array)))
              (ps:dolist (element (ps:chain *array(from (nyxt/ps:qsa root "*"))))
                (ps:chain elements (push element))
                (ps:when (ps:@ element shadow-root)
                  (get-all-elements (ps:@ element shadow-root) elements)))
              elements))
    (let ((elements (get-all-elements (ps:@ document)))
          (in-view-port-p (ps:lisp (eq :vi (hinting-type (find-submode 'hint-mode))))))
      (ps:dolist (element elements)
        (ps:when (ps:chain element (matches (ps:lisp selector)))
          (if in-view-port-p
