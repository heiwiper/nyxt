...
(defmethod prompter:object-attributes ((pkg ospm:os-package) (source prompt-buffer-source))
...
(defmethod prompter:object-attributes ((output ospm:os-package-output) (source prompt-buffer-source))
  (declare (ignore source))
...
(defmethod prompter:object-attributes ((gen ospm:os-generation) (source prompt-buffer-source))
  (declare (ignore source))
  `(("ID" ,(ospm:id gen))
...
(defmethod prompter:object-attributes ((pkg ospm:guix-package) (source prompt-buffer-source))
  (declare (ignore source))
...
(define-class os-profile-source (prompt-buffer-source)
...
(define-class os-generation-source (prompt-buffer-source)
  ((prompter:name "Packages")
...
(define-command-global switch-os-generation ()
  ...
  (let* ((profile (prompt1 :sources '(os-profile-source)
                           :prompt "Target profile")))
    (operate-os-package "Switching to generation..." #'ospm:switch-generation profile
